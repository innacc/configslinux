...
static void closeCb_clicked(GtkButton *close, gpointer user_data, GtkNotebook *notebook);
static void
...
static void
closeCb_clicked(GtkButton *close, gpointer user_data, GtkNotebook *notebook)
{
	(void)close;
	struct Client *browser = (struct Client *)user_data;

	webkit_web_view_try_close(browser->webView);
		if(gtk_notebook_get_n_pages(GTK_NOTEBOOK(Window->main_window)) == 3)
		{
			exit(0); 
		}
